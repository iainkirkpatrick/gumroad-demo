      ...
      <div className="py-16 flex flex-col items-center">
        <div className="flex flex-col bg-white border border-black rounded-md min-w-96">
          ...
          {editableCart && editableCart.cart_items.map((cartItem: any, i: number) => (
            <div className={`p-4 flex flex-col items-center gap-4 ${i !== cart.cart_items.length - 1 ? 'border-b border-black' : ''}`}>
        ...
        <div className="flex flex-col grow border border-black rounded-md bg-white">
          {editableCart && editableCart.cart_items.map((cartItem: any, i: number) => {
            if (cartItem.product.native_type === 'coffee') {
              ...
              return (
                <div className='p-4 flex flex-col gap-4 border-b border-black'>
                  ...
                  <div className="flex flex-col grow border border-black rounded-md bg-white">
                    <CartItem
                      ...
                      isLast={i === cart.cart_items.length - 1}
                      renderConfigure={() => (
                        <ConfigureWrapper
                          renderChildren={({ isOpen, setIsOpen }) => (
                            <>
                              <button
                              ...
                              {isOpen && (
                                <div className='p-4 absolute top-full flex flex-col gap-4 min-w-96 border border-black rounded-md bg-white shadow-md'>
                                  ...
                                  <button
                                    ...
                                    onClick={() => setEditableCart({ ...editableCart, cart_items: editableCart.cart_items.map(ci => ci.product.public_id === cartItem.product.public_id ? { ...ci, variant_id: selectedVariant.id } : ci) })}
                                  >Save changes</button>
                                </div>
                              )}
                            </>
...
interface ConfigureWrapperProps {
  // open?: boolean;
  // onOpen?: () => void;
  renderChildren: ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (val: boolean) => void }) => React.ReactNode;
  className?: string;
}
...
function CartItem ({
  ...
  return (
    <div className={`flex ${!isLast ? 'border-b border-black' : ''}`}>
      ...
      <div className="p-4 flex justify-between w-full">
        <div className="flex flex-col justify-between">
          ...
          <div className='flex gap-4'>
            ...
            {variantName && <p className="text-sm"><span className="font-bold">Variant: </span>{variantName}</p>}
          </div>
        </div>
